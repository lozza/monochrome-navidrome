...
export class UIRenderer {
    ...
    async showPage(pageId) {
        ...
        document.querySelectorAll('.sidebar-nav a').forEach((link) => {
            const isHome =
                link.pathname === '/' && (window.location.pathname === '/' || window.location.pathname === '/home');
            const isCurrent = link.pathname === window.location.pathname && link.search === window.location.search;
    ...
    activateLibraryTab(tabId) {
        ...
        const tab = page.querySelector(`.search-tab[data-tab="${CSS.escape(tabId || '')}"]`);
        if (!tab) return false;

        page.querySelectorAll('.search-tab').forEach((item) => item.classList.remove('active'));
        page.querySelectorAll('.search-tab-content').forEach((item) => item.classList.remove('active'));
        tab.classList.add('active');
        document.getElementById(`library-tab-${tab.dataset.tab}`)?.classList.add('active');
        return true;
    }

    async renderLibraryPage(selectedTab = null) {
        ...
        singlesContainer.innerHTML = this.createSkeletonCards(12, false);
        let renderedAlbums = 0;
        let renderedSingles = 0;
        ...
        const appendAlbumBatch = async (batch) => {
            const singles = batch.filter(isSingle);
            const albums = batch.filter((album) => !isSingle(album));

            if (albums.length) {
                if (renderedAlbums === 0) albumsContainer.innerHTML = '';
                albumsContainer.insertAdjacentHTML(
                    'beforeend',
                    albums.map((album) => this.createAlbumCardHTML(album)).join('')
                );
                renderedAlbums += albums.length;
            }
            if (singles.length) {
                if (renderedSingles === 0) singlesContainer.innerHTML = '';
                singlesContainer.insertAdjacentHTML(
                    'beforeend',
        ...
        const [allAlbums, allArtists, navidromePlaylists] = await Promise.all([
            this.api.getAllAlbums(250, appendAlbumBatch).catch((error) => {
                albumsContainer.innerHTML = createPlaceholder(`Could not load albums: ${error.message}`);
                singlesContainer.innerHTML = createPlaceholder(`Could not load singles: ${error.message}`);
                return null;
            }),
        ...
        if (allAlbums?.length) {
            if (!renderedAlbums) albumsContainer.innerHTML = createPlaceholder('No albums found in Navidrome.');
            if (!renderedSingles) singlesContainer.innerHTML = createPlaceholder('No singles found in Navidrome.');
        } else if (allAlbums) {
        ...
        // When connected to Navidrome, the server playlist section above is
        // the single source of truth. Do not render a second IndexedDB copy in
        // "My Playlists"; keep that area for the create action and legacy
        // unconfigured/local mode only.
        const myPlaylists = db.usesRemotePlaylists?.() ? [] : await db.getPlaylists();
        ...
        if (selectedTab) this.activateLibraryTab(selectedTab);
    }
    ...
    async renderPlaylistPage(playlistId, source = null, _provider = null) {
        ...
        try {
            ...
            // Navidrome playlists are editable server-owned playlists, even
            // when they were opened from the normal /playlist/:id route. Load
            // them through the database facade so the same render path powers
            // create/edit/remove/reorder actions without a local copy.
            if (db.usesRemotePlaylists?.() && !String(playlistId).startsWith('VL')) {
                try {
                    playlistData = await db.getPlaylist(playlistId);
                    ownedPlaylist = playlistData;
                } catch (error) {
                    console.warn('Failed to load Navidrome playlist:', error);
                }
            }
