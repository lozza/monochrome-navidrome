...
const syncManager = {
    ...
    async syncUserPlaylist(playlist, action) {
        // Navidrome-owned playlists are already persisted server-side. Do not
        // mirror them into the old hosted PocketBase account service.
        if (db.usesRemotePlaylists?.() || playlist?.isNavidromePlaylist) return;
